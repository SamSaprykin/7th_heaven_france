...
import maskTissus from '../../data/images/categories/maskTissus.png'

export const query = graphql`
query masksTissus{
    allMarkdownRemark(
        limit:9,
        filter: {fileAbsolutePath: {regex: "\/product/"},frontmatter: {category: {eq: "masques-tissue"}}}
...
export default class MaskTissus extends React.Component {
    render() {
        const maskTissus = get(this, 'props.data.allMarkdownRemark.edges')
        return (
            <Layout location={this.props.location}>
            <Helmet
                htmlAttributes={{ lang: 'en' }}
                meta={[{ name: 'description', content: "MASQUES TISSUS"}]}
                title="7th Heaven - MASQUES TISSUS"
            />   
            <BigHeader style={{zIndex:`-100`}}>
                <WrapperImages>
                    <TextElle>MASQUES TISSUS</TextElle>
                </WrapperImages>            
            </BigHeader>
            <ProductsWrapper>
    ...
    width: 100%;
    background-image: url(${maskTissus});
    height: 600px;
