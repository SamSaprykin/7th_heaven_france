...
import lesPatches from '../../data/images/categories/lesPatches.png'
...
query lePatches{
  allMarkdownRemark(
      limit:9,
      filter: {fileAbsolutePath: {regex: "\/product/"},frontmatter: {category: {eq: "strips"}}}
...
export default class LePathes extends React.Component {
    render() {
        const lePatches = get(this, 'props.data.allMarkdownRemark.edges')
        return (
            <Layout location={this.props.location}>
            <Helmet
                htmlAttributes={{ lang: 'en' }}
                meta={[{ name: 'description', content: "LES PATCHES"}]}
                title="7th Heaven - LES PATCHES"
            />   
            <BigHeader style={{zIndex:`-100`}}>
                <WrapperImages>
                    <TextElle>LES PATCHES</TextElle>
                </WrapperImages>            
            </BigHeader>
            <ProductsWrapper>
...
width: 100%;
background-image: url(${lesPatches});
height: 600px;
