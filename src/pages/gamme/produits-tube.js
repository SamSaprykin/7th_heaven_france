...
import procuitTubes from '../../data/images/categories/procuitTubes.png'

export const query = graphql`
query productsTube{
    allMarkdownRemark(
        limit:9,
        filter: {fileAbsolutePath: {regex: "\/product/"},frontmatter: {category: {eq: "produits-tube"}}}
...
export default class ProductsTube extends React.Component {
    render() {
        const productsTube = get(this, 'props.data.allMarkdownRemark.edges')
        return (
            <Layout location={this.props.location}>
            <Helmet
                htmlAttributes={{ lang: 'en' }}
                meta={[{ name: 'description', content: "PRODUITS EN TUBES"}]}
                title="7th Heaven - PRODUITS EN TUBES"
            />  
            <BigHeader style={{zIndex:`-100`}}>
                <WrapperImages>
                    <TextElle>PRODUITS EN TUBES</TextElle>
                </WrapperImages>            
            </BigHeader>
            <ProductsWrapper>
                 
                  {productsTube.map(({ node }) => {
    ...
    width: 100%;
    background-image: url(${procuitTubes});
    height: 600px;
