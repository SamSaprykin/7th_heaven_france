...
import maskSpeciaux from '../../data/images/categories/maskSpeciaux.png'

export const query = graphql`
query masksSpecial{
    allMarkdownRemark(
        limit:9,
        filter: {fileAbsolutePath: {regex: "\/product/"},frontmatter: {category: {eq: "masques-speciaux"}}}
...
export default class MasksSpecial extends React.Component {
    render() {
        const masksSpecial = get(this, 'props.data.allMarkdownRemark.edges')
        return (
            <Layout location={this.props.location}>
            <Helmet
                htmlAttributes={{ lang: 'en' }}
                meta={[{ name: 'description', content: "MASQUES SPECIAUX"}]}
                title="7th Heaven - MASQUES SPECIAUX"
            />  
            <BigHeader style={{zIndex:`-100`}}>
                <WrapperImages>
                    <TextElle>MASQUES SPECIAUX</TextElle>
                </WrapperImages>            
            </BigHeader>
            <ProductsWrapper>
                 
                  {masksSpecial.map(({ node }) => {
    ...
    width: 100%;
    background-image: url(${maskSpeciaux});
    height: 600px;
