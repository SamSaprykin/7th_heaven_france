...
import maskPeelOff from '../../data/images/categories/maskPeelOff.png'
...
query maskPeelOff{
    allMarkdownRemark(
        limit:9,
        filter: {fileAbsolutePath: {regex: "\/product/"},frontmatter: {category: {eq: "masques peel-off"}}}
...
export default class MasksPeelOff extends React.Component {
    render() {
        const maskPeelOff = get(this, 'props.data.allMarkdownRemark.edges')
        return (
            <Layout location={this.props.location}>
            <Helmet
                htmlAttributes={{ lang: 'en' }}
                meta={[{ name: 'description', content: "MASQUES PEEL-OFF"}]}
                title="7th Heaven - MASQUES PEEL-OFF"
            />  
            <BigHeader style={{zIndex:`-100`}}>
                <WrapperImages>
                    <TextElle>MASQUES PEEL-OFF</TextElle>
                </WrapperImages>            
            </BigHeader>
            <ProductsWrapper>
                 
                  {maskPeelOff.map(({ node }) => {
    ...
    width: 100%;
    background-image: url(${maskPeelOff});
    height: 600px;
