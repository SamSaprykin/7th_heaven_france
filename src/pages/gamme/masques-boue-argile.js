...
import maskBoue from '../../data/images/categories/mask-boue.png'

export const query = graphql`
query maskBoue{
    allMarkdownRemark(
        limit:9,
        filter: {fileAbsolutePath: {regex: "\/product/"},frontmatter: {category: {eq: "masques-boue-argile"}}}
...
export default class MasksBoue extends React.Component {
    render() {
        const maskBoue = get(this, 'props.data.allMarkdownRemark.edges')
        return (
            <Layout location={this.props.location}>
            <Helmet
                htmlAttributes={{ lang: 'en' }}
                meta={[{ name: 'description', content: "MASQUES A LA BOUE"}]}
                title="7th Heaven - MASQUES A LA BOUE"
            />  
            <BigHeader style={{zIndex:`-100`}}>
                <WrapperImages>
                    <TextElle>MASQUES A LA BOUE </TextElle>
                </WrapperImages>            
            </BigHeader>
            <ProductsWrapper>
